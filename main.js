...
function selectItem(employee, li) {
  //clear selection before selecting anything
  clearSelection();
  //atribute employee to the selected item
  selectedItem = employee;
  //adding the selected class to li selected
  li.classList.add("selected");

  //preencher os campos do item que eu selecionei no formulário
...
async function onDelete() {
  if (selectedItem) {
    const deletedItem = await deleteEmployee(selectedItem.id);
    const i = employees.indexOf(selectedItem);
